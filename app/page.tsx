...
export default function Home(){
  ...
  useEffect(()=>{if(!mount.current)return;const host=mount.current;
    ...
    const kd=(e:KeyboardEvent)=>{const k=e.key.toLowerCase();if(k.startsWith("arrow"))e.preventDefault();if(k==="arrowup")active.current.add("forward");if(k==="arrowdown")active.current.add("back");if(k==="arrowleft")active.current.add("left");if(k==="arrowright")active.current.add("right");if(k===" "){e.preventDefault();jump()}if(k==="f")shoot();if(k==="e")build();if(k==="r")remove()};const ku=(e:KeyboardEvent)=>{const k=e.key.toLowerCase();if(k==="arrowup")active.current.delete("forward");if(k==="arrowdown")active.current.delete("back");if(k==="arrowleft")active.current.delete("left");if(k==="arrowright")active.current.delete("right")};
  ...
  return <main className="game-shell"><div ref={mount} className="world"/>
    ...
    {help&&started&&<div className="guide"><div><button onClick={()=>setHelp(false)}>×</button><p>FIELD CONTROLS</p><h2>MOVE LIKE A FIGHTER.<br/>BUILD LIKE A CREATOR.</h2><section><span><kbd>ARROW KEYS</kbd> WALK</span><span><kbd>DRAG</kbd> LOOK</span><span><kbd>SPACE</kbd> JUMP</span><span><kbd>F</kbd> FIRE</span><span><kbd>E</kbd> BUILD</span><span><kbd>R</kbd> MINE</span></section><small>On phones, drag the world to look and use the on-screen controls.</small></div></div>}
  </main>
}
